<?php
	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		if( (empty($_POST["wsm_in"])) || ($_POST["wsm_in"] == "NONE") ){
			$wsm_in_err = "Please choose a workspace member.";     
		} else{
			$wsm_in = $_POST["wsm_in"];
		}
		...
		if(empty($wsm_in_err)){
			...
			try{
				...
				list($email_in, $ws_in) = explode("@@@@", $wsm_in);
				
				$b = $mysqli->query("DELETE FROM channel_members WHERE (workspace_id = '$ws_in') AND (email = '$email_in')");
				if(!$b){
					throw new Exception();
				}
				
				$c = $mysqli->query("DELETE FROM workspace_members WHERE (workspace_id = '$ws_in') AND (email = '$email_in')");
				if(!$c){
					throw new Exception();
				}
				
				try{
					$d = $mysqli->query("DELETE FROM workspace_admins WHERE (workspace_id = '$ws_in') AND (email = '$email_in')");
				} catch(Exception $f){}
				...
				$mysqli->rollback();
				header('Refresh: 2; URL = remove_workspace_users.php');
				die("<h2 style=\"font-weight:bold;text-align:center;color:red;\">COULD NOT DELETE THIS MEMBER</h2>
				<h3 style=\"font-weight:bold;text-align:center;\">Redirecting to remove workspace users page</h3>");
			}
			$mysqli->close();
			
			if($removed){
				header('Refresh: 2; URL = remove_workspace_users.php');
				die("<h2 style=\"font-weight:bold;text-align:center;color:green;\">Successfully removed member</h2>
				<h3 style=\"font-weight:bold;text-align:center;\">Redirecting to remove workspace user page</h3>");
			}
...
<div class="bs-example">
	...
	<div class="container-fluid">
		<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
			<br>
			<h3 class="text-center">Removing Users From a Workspace<h6 class="text-center">(Requires Workspace Admin Privilege)</h6></h3>
			<?php 
				$workspace_members = $mysqli->query("SELECT wsm.workspace_id, ws.workspace_name, ws.description, wsm.email, u.u_nickname,
											...
											AND (wsm.email != ws.workspace_owner)
											AND (ws.workspace_id IN (SELECT workspace_id FROM workspace_admins WHERE email = '$email'))
											");
			?>
			<div class="form-group <?php echo (!empty($wsm_in_err)) ? 'has-error' : ''; ?>">
			  <label class="col-md control-label" for="wsm_in"><h6><strong>Select a Workspace - User</strong></h6></label>
			  <div class="col-md">
					<select  class="selectpicker form-control" data-live-search="true" name="wsm_in">
					  <option></option>
					  <?php
					...
					<span class="help-block text-danger"><?php echo $wsm_in_err . "<br>"; ?></span>
